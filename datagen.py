...
def save_in_parts(df, prefix, num_parts=3, output_dir="."):
    os.makedirs(output_dir, exist_ok=True)
    chunks = np.array_split(df, num_parts)
    for i, chunk in enumerate(chunks):
        path = os.path.join(output_dir, f"{prefix}_node_{i}.csv")
...
def main(args):
    ...
    save_in_parts(left_df, "left", num_parts=args.num_parts, output_dir=args.output_dir)
    ...
    save_in_parts(right_df, "right", num_parts=args.num_parts, output_dir=args.output_dir)
